---
title: 使用Docker部署并持久化MySQL5.7
description: 使用 Docker 部署并持久化 MySQL 5.7 数据库。
publishDate: 2018-08-19 11:55:24
tags:
  - docker
  - mysql
---
...
```bash
docker run -d -e MYSQL_ROOT_PASSWORD=root --name mysql -v /opt/mysql/my.cnf:/etc/mysql/my.cnf -v /opt/mysql/data:/var/lib/mysql -p 3306:3306 mysql:5.7
```
...
```
1. update mysql.user set authentication_string=password('newpwd') where user='root';
2. flush privileges;
...
如果遇到mysql无法启动的情况，请检查 /opt/mysql 文件夹权限。
可`sudo chmod 777 -R /opt/mysql`后重启服务，`docker restart mysql`
